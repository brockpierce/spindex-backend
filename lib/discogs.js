...
async function resolveDiscogs(album) {
  ...
  if (!out) {
    ...
    await sleep(1100);
    // Search cover_image is often a placeholder/low-res. If we matched a master,
    // fetch it for the real cover art (and fuller genres/styles).
    if (out && out.masterId && !realImage(out.imageUrl)) {
      const m = await discogsFetchByRef({ kind: "master", id: out.masterId }, auth);
      await sleep(1100);
      if (m) {
        out.imageUrl = m.imageUrl;
        if (!out.genres.length) out.genres = m.genres;
        if (!out.styles.length) out.styles = m.styles;
      }
    }
  }
  if (!out) return null;
...
function enrichInBackground(albumId) {
  ...
  (async () => {
    try {
      ...
      // Fill a missing cover, OR upgrade a flaky Cover Art Archive (archive.org)
      // cover to the reliable Discogs image. Never overwrite a cover that's
      // already a good non-archive.org source.
      const isCaa = (u) => u && /archive\.org/i.test(u);
      if (result && result.imageUrl && (!album.coverArtUrl || album.coverArtUrl === "none" || isCaa(album.coverArtUrl))) {

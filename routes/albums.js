...
router.get("/", async (req, res) => {
  ...
  if (search) {
    try {
      // Sanitise for FTS5. Anything that isn't a letter, number or space is
      // QUERY SYNTAX to fts5 -- "Mr. Brightside" and "Hello, Goodbye" both
      // threw `fts5: syntax error near ","`. Tokens are quoted so they're
      // matched literally and never read as AND/OR/NOT/NEAR operators. Only
      // the last token gets a prefix * so partial typing still matches.
      const ftsTokens = search.replace(/[^\p{L}\p{N}\s]/gu, " ").split(/\s+/).filter(Boolean);
      const ftsQuery = ftsTokens
        .map((t, i) => (i === ftsTokens.length - 1 ? `"${t}"*` : `"${t}"`))
        .join(" ");
      const ftsResults = ftsTokens.length ? await prisma.$queryRawUnsafe(`
    ...
    // Live MusicBrainz fallback: fire only when local has no strong title match
    // for the term (so it catches new albums even by artists already in-catalog).
    // Fails soft -- never blocks or breaks local search.
    try {
      const localRows = Array.isArray(raw) ? raw.slice() : [];
      if (raw && raw._manualMatches) localRows.push(...raw._manualMatches);
      const fetched = await maybeFetch(search, localRows);

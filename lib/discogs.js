...
const { getCoverArtUrl } = require("./coverart");
...
function enrichInBackground(albumId) {
  ...
  (async () => {
    try {
      ...
      // Cover resolution — MusicBrainz is canonical, Discogs is a fallback ONLY
      // when CAA has DEFINITIVELY confirmed there's no MB art. We don't trust the
      // stored coverArtUrl to tell us that (on a first view it's still null, not
      // yet "none"), so we ask CAA ourselves for a definitive verdict, in-pass.
      const hasRealCover = album.coverArtUrl && album.coverArtUrl !== "none";
      if (!hasRealCover && album.musicbrainzId) {
        const caa = await getCoverArtUrl(album.musicbrainzId);
        if (caa.confirmed && caa.url) {
          data.coverArtUrl = caa.url;                 // MB has art -> canonical
        } else if (caa.confirmed && !caa.url) {
          // CAA confirms no MB art -> Discogs fallback if we found one, else "none".
          data.coverArtUrl = (result && result.imageUrl) ? result.imageUrl : "none";
        } else {
          // CAA was transient (timeout/5xx) — can't safely decide. Don't stamp
          // discogsCheckedAt, so a later view re-attempts both cover and tags
          // instead of freezing this album as a permanent placeholder.
          delete data.discogsCheckedAt;
        }
      }

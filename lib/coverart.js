...
async function getCoverArtUrl(musicbrainzReleaseGroupId) {
  ...
  try {
    // We use HEAD here, not GET -- we only need to know the final image
    // URL after the redirect, not the image bytes themselves. The browser
    // will fetch the actual image directly from Internet Archive when it
    // renders an <img> tag, so we don't need to download it ourselves.
    const response = await fetch(url, { method: "HEAD", redirect: "follow" });
    if (response.status === 404) return null; // no cover art exists for this release
    if (!response.ok) {
      console.warn(`Cover Art Archive returned ${response.status} for ${musicbrainzReleaseGroupId}`);
      return null;
    }
    return response.url; // the final, redirected image URL
  } catch (err) {
